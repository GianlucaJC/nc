...
$action="analisi_pr.php?id_ref=$id_ref";
...
<div class="risoluzione">
	...
	<form action="<?php echo $action; ?>" method="post" id='frm_ris' name='frm_ris' autocomplete="off" class="needs-validation2" novalidate >
		<div class="row mt-3">
			<div class="col-md-6">
			  ...
			  <div class="form-floating mb-3 mb-md-0">
				...
				<select class="form-select" id="attivita" aria-label="Attività" name='attivita' required <?php echo $dis; ?> onchange='set_rilav(this.value)'>
					<option value=''>Select...</option>
					<option value='1'
					<?php if ($info_nc_pr[0]['attivita']==1) echo " selected "; ?>
					>Accettare il prodotto</option>
					<option value='2'
					<?php if ($info_nc_pr[0]['attivita']==2) echo " selected "; ?>
					>Rilavorare il prodotto</option>
					<option value='3'
					<?php if ($info_nc_pr[0]['attivita']==3) echo " selected "; ?>
					>Selezionare ed eliminare i pezzi non conformi</option>
					<option value='4'
					<?php if ($info_nc_pr[0]['attivita']==4) echo " selected "; ?>
		...
		<?php include ("rilavorazione.php"); ?>	

		<div class='row mt-3'>
			<div class="col-md-12">
			  <label for="note">Note</label>
				<?php
					$note=stripslashes($info_nc_pr[0]['note']);
		...
		<?php
			$informazione_organizzazioni=stripslashes($info_nc_pr[0]['informazione_organizzazioni']);
	...
	?>
			...
			<div class='row mt-3'>
				<div class="col-md-2">
					<div class="form-floating mb-3 mb-md-0">
						<input class="form-control" id="data_sezione_ris1" name='data_sezione_ris1' type="date" required value="<?php echo $info_nc_pr[0]['data_sezione_ris1'];?>" <?php echo $dis1; ?> />
						<label for="data_sezione_ris1">Data</label>
					</div>
				</div>
				<div class="col-md-6">

					<!-- vedi azione del submit in analisi_pr.js needs-validation2 !-->
					...
					<?php 
						//if ($_SESSION['nc_access']=="3") {
							$fxx="pdf_rilavorazioni/".$id_ref.".pdf";
							if (file_exists($fxx)) {
								echo "<a href='$fxx' target='_blank'>";
									echo "<button type='button' name='btn_pdf_ril' id='btn_pdf_ril' class='btn btn-info btn-lg ml-1'>";
										echo "<i class='fas fa-file-pdf'></i> PDF Rilavorazione";
									echo "</button>";
								echo "</a>";
							}
						//}
			...
			<?php
				...
				/*
				$vis="display:none";
				if ($info_nc_pr[0]['motivazione_azione']!=null && strlen($info_nc_pr[0]['motivazione_azione'])!=0) $vis="";
				*/
				$vis="";
			?>	
			...
			<div class='row mt-1'>
				  ...
				  <div class="mb-3 mb-md-0">
						<div class="form-floating mb-3 mb-md-0">
							<select class="form-select" name='azione_correttiva' id='azione_correttiva' data-placeholder="Azione correttiva" style="width: 100%;" required onchange='set_motivazione(this.value)' <?php echo $dis2; ?> >
								...
								<option value='' 
								<?php if ($info_nc_pr[0]['azione_correttiva']==null) echo " selected "; ?>
								
								>Select...</option>
								<option value=1
								<?php if ($info_nc_pr[0]['azione_correttiva']=="1") echo " selected "; ?>
								>SI</option>
								<option value=0
								<?php if ($info_nc_pr[0]['azione_correttiva']=="0") echo " selected "; ?>
								>NO</option>
							</select>
				...
				<div class="col-md-2">
					<div class="form-floating mb-3 mb-md-0">
						<input class="form-control" id="data_sezione_ris2" name='data_sezione_ris2' type="date" value="<?php echo $info_nc_pr[0]['data_sezione_ris2'];?>"  <?php echo $dis2; ?> />
			...
			<div class='row mt-3' style='<?php echo $vis; ?>' id='div_motivazione_azione'>
				<div class="col-md-12">
				  ...
				  <div class="mb-3 mb-md-0">
					<textarea class="form-control" id="motivazione_azione" name="motivazione_azione" rows="2" <?php echo $dis2; ?>><?php echo stripslashes($info_nc_pr[0]['motivazione_azione'])?></textarea>
