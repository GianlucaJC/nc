...
$(function(){
	...
	check_sign_valutazione1=$("#check_sign_valutazione1").val();
	
	check_sign_valutazione2=$("#check_sign_valutazione2").val();
	if ((check_sign_valutazione1 && check_sign_valutazione1!="0") || (check_sign_valutazione2 && check_sign_valutazione2!="0"))
...
function sign_valutazione(id_ref,from) {
	
	if (from=="1") {
		data=$("#data_valutazione1_nc").val();
		if (!data || data.length==0) {
			alert("Definire correttamente la data!");
			return false;
		}
	}	
	if (from=="2") {
		data=$("#data_valutazione2_nc").val();
		if (!data || data.length==0) {
			alert("Definire correttamente la data!");
			return false;
		}
	...
	if (!confirm("Sicuri di apporre la firma (l'operazione è irreversibile)?")) return false;
	
	$("#submit_sign_valutazione").val(from)
	$("#frm_val").submit();
...
function set_rilav(value){
	$("#div_rilavorazione").hide(100);
	if (value=="2") {
		$(".obbl").prop("required",true);
		$("#div_rilavorazione").show(100);
		
	} else {
		$(".obbl").removeAttr("required");
	}		
}
